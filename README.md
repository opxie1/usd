...
`scripts/task23_rate_system_var.py` runs Tasks 2 and 3 (which rates respond to the money supply; which rates drive others) for the implied-rate system. Because the implied rates are not cointegrated in levels (see `task23_diff_and_johansen.py`), this is now estimated in first differences: a six-variable VAR of the five differenced sector rates (federal, state/local ex-pension, mortgage, business, consumer) plus M2-less-base growth. AIC lag selection capped at 8, stability and minimum root modulus reported, full pairwise Granger matrix plus joint tests of the federal-rate hierarchy, and IRFs to federal-rate and money shocks. In differences the federal rate still jointly causes the other rates (p about 0.04) but with feedback. The market-rate system is cointegrated, so its hierarchy is estimated with a VECM in `task23_diff_and_johansen.py`, where the one-way ordering is clean. Outputs in `tables/task23_granger_matrix_*.csv`, `task23_var_meta.csv`, `figures/task23_*.png`. Interpretation caveat: four of the five implied rates are backward-looking weighted averages, so comparisons with the market mortgage rate mix a current rate with averages.
...
`scripts/task4_vecm_spec_compare.py` estimates the Task 4 VECM under both deterministic specifications (constant only, rank 2 from the trace test; constant plus trend, rank 4) and saves log-likelihoods, loadings, cointegrating vectors, and residual diagnostics side by side in `tables/task4_vecm_specs_*.csv` so the specification choice can be made on economic grounds. The VECM residuals are white at 5 percent (whiteness p about 0.08 and 0.11); normality is rejected, as is common for macroeconomic data.
