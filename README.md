...
`scripts/task5b_sector_credit_demand.py` runs the per-sector extension of Task 5 requested in review. For each private-debt type (household mortgages, consumer credit, nonfinancial corporate) it regresses that debt's growth on its own interest rate (lagged one quarter) and lagged money growth, estimating the money coefficient two ways (Kalman-smoothed random walk and rolling 40-quarter OLS, both with 95 percent bands) and under two money measures (M2-less-base and base money). It does this for both the market rate and the paper's implied rate as the sector's own rate; the market-rate version is the cleaner of the two because the implied rate carries that sector's debt in its denominator, so pairing it with the same debt's growth can induce a mechanical correlation (mortgage has no implied rate, so it reuses the market mortgage rate in both runs). Outputs in `tables/task5b_*.csv` and `figures/task5b_*.png` (market-rate figures unsuffixed, implied-rate figures suffixed `_implied`). First-pass reading: consumer borrowing carries a positive coefficient on M2-less-base growth that is marginal under the market rate and significant under the implied rate (the latter likely overstated by the mechanical link), while corporate borrowing does not respond and turns negative after 2020 under both rate definitions, consistent with a credit-demand mechanism that is weaker where borrowing funds value creation. Base money is mostly insignificant across sectors and both rate types, with only mortgage borrowing showing intermittent base-money sensitivity in recent windows, so the lingering-reserves channel gets weak and mixed support. Several Kalman specifications drove the coefficient's state variance to zero, meaning the data did not support time variation and the smoothed coefficient is effectively constant; the rolling OLS is the more informative view in those cases.
